...
const filterOpenBtn = document.querySelector('.products__filter-btn'),
      filterCloseBtn = document.querySelector('.filter__close-btn'),
      filterSubmitBtn = document.querySelector('.filter__submit-btn'),
      filterHiddenCheckbox = document.querySelectorAll('.filter__checkbox-hidden'),
      filterCheckedItem = document.querySelectorAll('.filter__item-checked'),
...
item.forEach((oneItem) => {
  ...
  const overlay = oneItem.querySelector('.item__overlay'),
        ...
        openModalBtn = oneItem.querySelector('.item__more'),
  ...
  */
  ...
  /*
  при клике на item__more открывается модальное окно
  */
...
const swiper = new Swiper('.modal-item__slider', {
  grabCursor: true,
  slidesPerView: 4.5,
  spaceBetween: 3,
});
...
*/
const formBtn = document.querySelector('.products__show-form'),
      searchForm = document.querySelector('.products__content-search'),
      formWrapper = document.querySelector('.products__search-wrapper');

/*
по клику на иконку лупы, открывается форма поиска
*/
formBtn.addEventListener('click', () => {
  searchForm.classList.toggle('products__content-search--active');
});
...
document.addEventListener('click', function (e) {
  if (e.target !== formWrapper) {
    searchForm.classList.remove('products__content-search--active');
  };
});
/*
